...
markList767.addEventListener("click", (e) => {
  ...
  filters767.setAttribute("style", "display:block !important");
...
markInput767.addEventListener("input", (e) => {
  const searchValue = e.target.value;
  const marksLowerCase = marksArr.map((a) => a.name.toLowerCase());
  const newMarks = marksLowerCase.filter((a) => a.includes(searchValue));
  if (newMarks.length) {
    markList767.textContent = "";
    newMarks.map((a) => {
      const li = document.createElement("li");
